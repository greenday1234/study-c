...
void add_rear(DequeType *q, element item){      //기존 큐 코드
    if(is_full(q))
    error("큐가 포화상태입니다.");
...
element delete_front(DequeType *q){             //기존 큐 코드
    if(is_empty(q))
    error("큐가 공백상태입니다.");
...
element get_front(DequeType *q){    //맨 앞(front)의 값을 보이는 함수
    if(is_empty(q))
    error("큐가 공백상태입니다.");
    return q->data[q->front + 1 % MAX_QUEUE_SIZE];   //주의할 점은 q->front = q->front + 1이 아니기때문에 front의 값은 변하지 않는다.
}                 
